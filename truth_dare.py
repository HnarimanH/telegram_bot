...
class Questions:
    def __init__(self):
        try:
            self.df = pd.read_csv("questions.csv")  # Ensure the file name is correct
            print("CSV file loaded successfully.")
        except FileNotFoundError:
            print("Could not find 'questions.csv'.")
            self.df = None  # Prevents errors if file loading fails
    
    def choose_random(self, q_type):
        if self.df is None:
            return "No questions available (CSV file missing)."
        
        if "Type" not in self.df.columns or "English Question" not in self.df.columns:
            return "Invalid CSV format: Missing required columns."
        
        questions = self.df[self.df["Type"] == q_type]["English Question"].tolist()
...
class Bot:
    ...
    async def join_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        elif data_base.select_name(group1,name) == None:
    ...
    async def leave_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        group1 = update.message.chat.title
        name = update.message.from_user.first_name
        db_name = data_base.select_name(name, group1)
    ...
    async def game_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # player,question = await self.choose_pq(all_players=all_players)
        print(await self.choose_pq(all_players=all_players))
    ...
    async def T_or_D(self,update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async def choose_pq(self,all_players):
        player = random.choice(all_players)
        q_type = await self.T_or_D()
        question = data_q.choose_random(q_type)
